...
* **单文件单核**：`rls.exe` 内链同一套核心逻辑，CLI 与 MCP 只是两种入口——同名请求、同默认值、同游标绑定、同元数据级别。人在终端看到的和 AI 拿到的**是同一套结果**，不漂移。
* **MCP 自带用法契约**：服务启动就把“定位→深读→按需追踪”工作流、证据等级（confirmed / candidate / unresolved / gap / truncated）、出错恢复步骤直接教给 AI。工具不只给能力，还给**正确用法**——新换的 AI 也能一次用对。
* **新鲜度铁律**：索引按 `mtime + 体积 + git HEAD` 判定过期，命中则增量更新；契约只有一条——**宁可多花时间，绝不用旧索引返回结果**。
* **预热只加速、不决断**：后台低优先级预热（启动 30 s 后、15 分钟发现、5 s 防抖），文件变化只标记脏；每次前台请求仍走同一刷新路径做最终确认。预热挂了、慢了，都不影响结果正确性，只影响快慢。
